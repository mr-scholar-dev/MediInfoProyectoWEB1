...
export function AdditionalDetailPage() {
  ...
  async function toggle() {
    if (!item) return
    setChanging(true)
    try { await api.patch(`/servicios-adicionales/${item.id}/estado`, { activo: item.activo === false }); navigate(0) } catch (cause) { setError((cause as Error).message) } finally { setChanging(false) }
  }
